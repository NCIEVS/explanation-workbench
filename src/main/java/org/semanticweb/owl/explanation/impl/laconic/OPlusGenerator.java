...
//        result.add(dataFactory.getOWLSubClassOfAxiom(nominal, type));
    ...
    public Set<? extends OWLAxiom> visit(OWLInverseObjectPropertiesAxiom axiom) {
        // Implication
        Set<OWLAxiom> result = new HashSet<OWLAxiom>();
        result.add(dataFactory.getOWLSubObjectPropertyOfAxiom(axiom.getFirstProperty(), dataFactory.getOWLObjectInverseOf((OWLObjectProperty) axiom.getSecondProperty())));
        result.add(dataFactory.getOWLSubObjectPropertyOfAxiom(axiom.getSecondProperty(), dataFactory.getOWLObjectInverseOf((OWLObjectProperty) axiom.getFirstProperty())));
        return log(axiom, result);
    }
